...
class System:
    def __init__(self):
        ...
        self.instances = {}        #key - instance name, value - model
    ...
    def find_templates(self, instace_name : str) -> str:
        try:
            model_str : str = self.instances[instace_name]
            init_function = self.get_init_name(model_str)
            return instace_name + "." + init_function
        except:
            print("Wrong instance name!")

    def get_init_name(self, model : Model) -> str:
        target_node : Node = model.start_state
        script : str = str(target_node.get_name()) + "()"
        return script

    def add_instance_info(self, full_inst : str, instance : str, model : Model):
        print("System added: " + instance + ", " + str(model.name))
        self.instance_declare.append(full_inst)
        self.instances[instance] = model
